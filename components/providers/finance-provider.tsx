...
export function FinanceProvider({ children }: { children: ReactNode }) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [categoriesRes, accountsRes, goalsRes, transactionsRes, rulesRes] = await Promise.all([
        supabase.from("categories").select("*").eq("user_id", user.id),
        supabase.from("accounts").select("*").eq("user_id", user.id),
        supabase.from("goals").select("*").eq("user_id", user.id),
        supabase.from("transactions").select("*").eq("user_id", user.id).order("date", { ascending: false }),
        supabase.from("auto_rules").select("*").eq("user_id", user.id),
      ])
        ...
        setRules(
          rulesRes.data.map((r: any) => ({
            ...
            trigger: {
              ...
              category: r.trigger_category || undefined,
            },
            action: {
              type: r.action_type,
              percentage: r.action_percentage || undefined,
              fixedAmount: r.action_fixed_amount || undefined,
              targetAccountId: r.action_target_account_id || undefined,
              targetGoalId: r.action_target_goal_id || undefined,
            },
  ...
  useEffect(() => {
    if (rulesLoaded && userId && rules.length >= 0) {
      const syncRulesToSupabase = async () => {
        try {
          // Delete all existing rules for this user and recreate them
          const { error: deleteError } = await supabase.from("auto_rules").delete().eq("user_id", userId)
          if (deleteError) {
            console.error("[v0] Error deleting old rules:", deleteError)
            return
          }
          ...
          if (rules.length > 0) {
            const rulesToInsert = rules.map((r) => ({
              ...
              trigger_category: r.trigger.category || null,
              action_type: r.action.type,
              action_percentage: r.action.percentage || null,
              action_fixed_amount: r.action.fixedAmount || null,
              action_target_account_id: r.action.targetAccountId || null,
              action_target_goal_id: r.action.targetGoalId || null,
              execution_count: r.executionCount || 0,
              last_executed: r.lastExecuted || null,
            }))

            const { error: insertError } = await supabase.from("auto_rules").insert(rulesToInsert)
            if (insertError) {
              console.error("[v0] Error syncing rules to Supabase:", insertError)
            }
          }
        } catch (error) {
          console.error("[v0] Error in syncRulesToSupabase:", error)
        }
      }

      syncRulesToSupabase()
    }
  }, [rules, rulesLoaded, userId, supabase])
  ...
  const addRule = useCallback(
    (rule: Omit<AutoRule, "id" | "executionCount">) => {
      ...
      if (userId) {
        supabase
          ...
          .catch((err) => console.error("[v0] Error adding rule to Supabase:", err))
      }
    },
    [userId, supabase],
  )

  const updateRuleFunc = useCallback(
    (id: string, updates: Partial<AutoRule>) => {
      ...
      if (userId) {
        supabase
          ...
          .update(updates)
          .eq("id", id)
          .catch((err) => console.error("[v0] Error updating rule in Supabase:", err))
      }
    },
    [userId, supabase],
  )

  const deleteRuleFunc = useCallback(
    (id: string) => {
      ...
      if (userId) {
        supabase
          ...
          .delete()
          .eq("id", id)
          .catch((err) => console.error("[v0] Error deleting rule from Supabase:", err))
      }
    },
    [userId, supabase],
  )
      ...
      for (const rule of enabledRules) {
        ...
        try {
          if (rule.action.targetAccountId) {
            ...
            setRules(updatedRules)
            localStorage.setItem("cashboard_auto_rules", JSON.JSON.stringify(updatedRules))
