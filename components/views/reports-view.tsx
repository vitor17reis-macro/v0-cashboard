...
export function ReportsView() {
  ...
  const yearlyData = useMemo(() => {
    try {
      ...
      console.error("[v0] Error calculating yearly data:", error)
      return []
    }
  }, [transactions])
  ...
  const categoryData = useMemo(() => {
    try {
      ...
      console.error("[v0] Error calculating category data:", error)
      return []
    }
  ...
  const trendData = useMemo(() => {
    try {
      ...
      console.error("[v0] Error calculating trend data:", error)
      return []
    }
  }, [transactions])
  ...
  const kpis = useMemo(() => {
    try {
      ...
      console.error("[v0] Error calculating KPIs:", error)
      return {
        thisIncome: 0,
        thisExpenses: 0,
        incomeChange: 0,
  ...
  const topExpenses = useMemo(() => {
    try {
      ...
      console.error("[v0] Error calculating top expenses:", error)
      return []
    }
  }, [transactions, categories])
  ...
  return (
    <div className="h-full flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border-emerald-500/20 hover-lift">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <TrendingUp className="h-5 w-5 text-emerald-600" />
              <div
                className={`flex items-center gap-1 text-xs font-medium ${kpis.incomeChange >= 0 ? "text-emerald-600" : "text-red-500"}`}
              >
        ...
        <Card className="bg-gradient-to-br from-red-500/10 to-red-500/5 border-red-500/20 hover-lift">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <CreditCard className="h-5 w-5 text-red-500" />
              <div
                className={`flex items-center gap-1 text-xs font-medium ${kpis.expenseChange <= 0 ? "text-emerald-600" : "text-red-500"}`}
              >
        ...
        <Card className="bg-gradient-to-br from-amber-500/10 to-amber-500/5 border-amber-500/20 hover-lift">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
        ...
        <Card className="bg-gradient-to-br from-blue-500/10 to-blue-500/5 border-blue-500/20 hover-lift">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mb-1">Progresso Metas</p>
      ...
      <Tabs defaultValue="overview" className="flex-1 flex flex-col min-h-0">
        ...
        <TabsContent value="trends" className="flex-1 overflow-auto space-y-4 mt-4">
          <Card>
            <CardHeader>
                    ...
                    <div key={item.name} className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <div className="h-8 bg-emerald-500/20 rounded-lg overflow-hidden">
                            <div
                              className="h-full bg-emerald-500 transition-all duration-500"
                              style={{ width: `${(item.receitas / maxTrendValue) * 100}%` }}
                            />
                          </div>
                          <div className="text-xs font-medium text-emerald-600 mt-1">{item.receitas.toFixed(0)}€</div>
                        </div>
                        <div>
                          <div className="text-xs text-muted-foreground mb-1">Despesas</div>
                          <div className="h-8 bg-red-500/20 rounded-lg overflow-hidden">
                            <div
                              className="h-full bg-red-500 transition-all duration-500"
                              style={{ width: `${(item.despesas / maxTrendValue) * 100}%` }}
                            />
                          </div>
                          <div className="text-xs font-medium text-red-500 mt-1">{item.despesas.toFixed(0)}€</div>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <TabsContent value="categories" className="flex-1 overflow-auto space-y-4 mt-4">
          <Card>
            <CardHeader>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Despesas mais significativas deste mês</CardDescription>
              </CardHeader>
              <CardContent>
                {topExpenses.length > 0 ? (
                  <div className="space-y-3">
                    {topExpenses.map((expense, index) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-full bg-red-500/20 flex items-center justify-center text-red-500 font-bold text-sm">
                            {index + 1}
                          </div>
                          <div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Estado atual das suas metas financeiras</CardDescription>
              </CardHeader>
              <CardContent>
                {goals.length > 0 ? (
                  <div className="space-y-4">
                  ...
                  <div className="py-8 text-center text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">Crie metas para acompanhar o progresso</p>
                  </div>
                )}
              </CardContent>
            </Card>
