...
export function HistoryView() {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "investment":
        return <TrendingUpIcon className="h-4 w-4 text-investment" />
      case "savings":
  ...
  const transactionsByType = useMemo(() => {
    ...
    filteredTransactions.forEach((t) => {
      const type = t.type as keyof typeof grouped
      if (grouped[type]) {
        grouped[type].push(t)
      }
    })
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="grid w-full grid-cols-6">
          ...
          {Object.entries(transactionsByType).map(([type, txs]) => {
            const TypeIcon = typeIcons[type as keyof typeof typeIcons]
            return (
              <TabsTrigger key={type} value={type} className="gap-2">
                <TypeIcon className="h-4 w-4" />
                <span className="hidden sm:inline">
                  {typeLabels[type as keyof typeof typeLabels]} ({txs.length})
                </span>
                <span className="sm:hidden">{txs.length}</span>
              </TabsTrigger>
            )
          })}
        </TabsList>
        ...
        {Object.entries(transactionsByType).map(([type, transactions]) => (
          <TabsContent key={type} value={type} className="space-y-3">
            <Card className="bg-card/50 backdrop-blur-sm border-border/50">
              <CardHeader>
                <CardTitle className="text-base">{typeLabels[type as keyof typeof typeLabels]}</CardTitle>
                <CardDescription>
                  {transactions.length} {transactions.length === 1 ? "transação" : "transações"} • Arraste para
                  reclassificar ou clique em "Reverter" para anular.
                </CardDescription>
              </CardHeader>
              <CardContent>
