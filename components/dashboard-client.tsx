...
export function DashboardClient({ user }: DashboardClientProps) {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case VIEWS.COMPARISON:
        return <DashboardView /> // Will render comparison tab within dashboard
  ...
  return (
    <div className="min-h-screen bg-background transition-colors duration-300">
        ...
        <div className="max-w-7xl mx-auto px-8 h-18 flex items-center justify-between py-4">
          <div className="flex items-center gap-10">
            ...
            <nav className="hidden lg:flex items-center gap-2">
              <NavButton view={VIEWS.DASHBOARD} icon={LayoutDashboardIcon} label="Visão Geral" />
              <NavButton view={VIEWS.HISTORY} icon={HistoryIcon} label="Histórico" />
              <NavButton view={VIEWS.COMPARISON} icon={GitCompareArrows} label="Comparação" />
              <NavButton view={VIEWS.SUBSCRIPTIONS} icon={CalendarClockIcon} label="Assinaturas" />
            </nav>
          </div>
      ...
      <main className="max-w-7xl mx-auto p-8 min-h-[calc(100vh-72px)]">{renderView()}</main>
